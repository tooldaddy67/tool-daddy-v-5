...
export default function BenchmarkBuilderClient() {
    ...
    const runBenchmark = async () => {
        ...
        if (isNaN(iter) || iter <= 0) {
            toast({ title: 'Invalid iterations', variant: 'destructive' });
            setRunning(false);
            return;
        }
        ...
        if (iter > MAX_ITERATIONS) {
            toast({
                ...
                description: `Capping iterations at ${MAX_ITERATIONS.toLocaleString()} to prevent browser freeze.`,
                variant: 'destructive'
            });
            iter = MAX_ITERATIONS;
            setIterations(MAX_ITERATIONS.toString());
        }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="p-6 rounded-2xl bg-red-500/10 border border-red-500/30 flex gap-4">
                    ...
                    <div className="space-y-1">
                        <p className="text-sm font-bold text-red-500">Security Warning</p>
                        <p className="text-xs text-muted-foreground leading-relaxed">
                            This tool executes JavaScript in your browser. **Never paste code you don{"'"}t understand.**
                            Running untrusted snippets can compromise your local data or session.
                        </p>
                    </div>
                </div>
            </div>
